...
def ParseArgs():
    ...
    parser.add_argument(
        ...
        '--setting_yml_path',
        type=str,
        default='/home/kakeya/Desktop/higuchi/20191107/experiment/hist_equal05/setting.yml')
...
def preprocess_lesion_df(df):
    '''lesion_dfの前処理'''
    def preprocess(x): return int(x.split('.')[0])
...
def plot_each_lesion_dice(df, save_dir=None):
    for leison, c_col in zip(['kidney', 'CCRCC', 'Cyst'], ['count', 'count_CCR', 'count_cys']):
        ...
        df2 = df.query('existence==1 & label_name==@leison')[
            ['dice', 'cid', c_col]].sort_values('dice').reset_index(drop=True)
        ...
        for index, row in df2.iterrows():
            g.text(
                index,
                row.dice + 0.01,
                f'{str(row[c_col])[0]}e{len(str(int(row[c_col])))}',
                color='black',
                ha='center')
