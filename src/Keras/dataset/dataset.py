...
class Generator(Sequence):
    def __init__(self, dataset, batch_size=4, nclasses=1,
                 enable_random_crop=True, enable_random_flip=False, enable_random_norm=False, use_label=None,
    ...
    def binary_preprocess(self, Y):
        # 複数次元ラベルを0,1のbinary_xentropyに食わせられるラベルに変更。
        # 二値分類だけど、他の関数との整合性を考慮してone_hotにして返す
        # use_labelのリストで値1を持つ要素の右端のindexを取ってくる
        use_idx = [i for i, x in enumerate(self.use_label) if x == 1][-1]
        #print(Y.shape)
        #print(Y)
        one_Y = Y[..., use_idx]
        zero_Y = np.abs(one_Y - 1)
        new_Y = np.stack([one_Y, zero_Y],axis=4)
        # print(new_Y.shape)
        return new_Y
